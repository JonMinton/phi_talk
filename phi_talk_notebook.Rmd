...
obs_mean_diff <- 
  tidy_height %>% 
    ...
    summarise(mean_diff = mean_height[gender == "M"] - mean_height[gender == "F"]) %>% 
    pull(mean_diff)
...
mu_male      <- summaries %>% filter(gender == "M") %>% pull(mean_height)
sd_male      <- summaries %>% filter(gender == "M") %>% pull(sd_height  )
mu_female    <- summaries %>% filter(gender == "F") %>% pull(mean_height)
sd_female    <- summaries %>% filter(gender == "F") %>% pull(sd_height  )
